      ...
      await fetchEventSource(`${serverBaseURL}/sse`, {
        ...
        onopen(res) {
          if (res.ok && res.status === 200) {
            return Promise.resolve(console.log("Connection made ", res));
          } else if (
            res.status >= 400 &&
            res.status < 500 &&
            res.status !== 429
          ) {
            return Promise.resolve(console.log("Client side error ", res));
          }
          return Promise.resolve(console.log("Desconhecido ", res));
        },
        onmessage(event) {
